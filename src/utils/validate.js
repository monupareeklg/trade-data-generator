 ...
 * @throws {Error}
 */
function validatePair(pair, index = 0) {
  ...
  if (typeof pair.startPrice !== 'number' || isNaN(pair.startPrice)) {
    throw new Error(
      `${prefix}.startPrice must be a number. ` +
      `Got: ${pair.startPrice}`
    )
  }
  ...
  if (pair.volatility !== undefined) {
    if (typeof pair.volatility !== 'number' || isNaN(pair.volatility)) {
      throw new Error(
        `${prefix}.volatility must be a number. ` +
        `Got: ${pair.volatility}`
      )
    }
  ...
  if (pair.trend !== undefined) {
    if (typeof pair.trend !== 'number' || isNaN(pair.trend)) {
      throw new Error(
        `${prefix}.trend must be a number. Got: ${pair.trend}`
      )
    }
  ...
  if (
    pair.maxPrice !== undefined &&
    pair.maxPrice <= pair.startPrice * 0.1
  ) {
    throw new Error(
...
function _validatePairs(pairs) {
  ...
  if (pairs.length === 0) {
    throw new Error(
      '[MarketFeed] pairs cannot be empty. ' +
      'Provide at least one pair.'
    )
  }
  ...
  // Validate each pair
  pairs.forEach((pair, i) => validatePair(pair, i))
}
...
function _validateVolumeConfig(volume, prefix) {
  ...
  if (typeof volume.max !== 'number' || volume.max <= 0) {
    throw new Error(
      `${prefix}.volume.max must be a positive number. ` +
      `Got: ${volume.max}`
    )
  }
